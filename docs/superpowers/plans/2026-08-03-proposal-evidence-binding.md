...
The first two assertions preserve valid zero-proposal output. The last two prove
that each emitted proposal must include at least one evidence ID.
...
Expected: five failures because nested proposal `evidence_ids` is missing from
`required`. Any other result requires test diagnosis before production code
changes.
...
- Consumes: deduplicated `allowed_evidence_ids` and each recursively visited JSON
  Schema node.
- Produces: a required nested `evidence_ids` array with `minItems >= 1`, bounded
  items, and the existing controller allowlist enum.

- [ ] **Step 1: Require evidence and preserve its lower bound**

Inside each schema object containing a bound `evidence_ids` property, add the
field to that object's `required` list. Keep the existing lower-bound assignment
inside the evidence array branch:
