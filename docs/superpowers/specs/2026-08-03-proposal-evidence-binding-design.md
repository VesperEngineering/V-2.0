...
array is bound to non-empty controller evidence, preserve any existing lower
bound and enforce `minItems >= 1`. Keep the existing item enum.
...
- `vesper/platform/agent_runner.py`: add the nested schema lower bound inside the
  existing evidence-binding branch.
