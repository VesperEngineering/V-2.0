...
use crate::screens::{
    ScreenState, base_style, content_area_with_stale_notice, palette, panel, unavailable_message,
};
...
pub fn render_orders(frame: &mut Frame<'_>, area: Rect, view: &OrdersView, state: &ScreenState) {
    ...
    let owner_lines = if view.reconciliation_agents.is_empty() {
        vec![Line::from(
    ...
    frame.render_widget(
        Paragraph::new(owner_lines)
            .style(base_style(palette))
            .wrap(Wrap { trim: true })
            .block(panel("RECONCILIATION OWNER", palette)),
        sections[1],
    );
}
