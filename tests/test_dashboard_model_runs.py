...
def test_model_run_rows_include_baseline_trials_and_pending_candidate():
    state = {
        ...
        "accepted": [
            {"run": 2, "candidate": {"out_of_sample_ic": 0.04, "rank_ic": 0.04, "ranking_spread": 0.003}},
        ],
        "rejected": [
            {"run": 1, "candidate": {"out_of_sample_ic": 0.031, "rank_ic": 0.02, "ranking_spread": 0.001}},
        ],
    ...
    assert [(row["run"], row["status"]) for row in rows] == [
        (0, "BASELINE"), (1, "REJECTED"), (2, "ACCEPTED"), (3, "RUNNING"),
    ]
...
def test_dashboard_model_runs_button_shows_evidence_in_main_window():
    root = tk.Tk()
    root.withdraw()
    try:
